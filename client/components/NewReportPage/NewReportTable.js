...
export default withRouter(({ rows, batchId, history }) => {
  ...
  const getCourseUnitRealisationName = (entry) => {
    if (entry.missingEnrolment)
      return 'Missing enrollment'
    if (entry.type === 'EXTRA_ENTRY')
      return 'Erilliskirjaus'
  ...
  const MissingEnrollmentsInfo = () => onlyMissingEnrollments
    ? <Message style={styles.missingEnrolmentInfo} warning>
      The report contains only completions with missing enrollments and nothing will be sent to Sisu.<br />
      When a student enrolls to the course the completion will be sent automatically to Sisu.
    </Message>
    : null

  const CompletionDate = ({ attainmentDate, completionDate }) => completionDate
    ? <div style={styles.completionDate}>
      <span>{moment(completionDate).format("DD.MM.YYYY")}</span>
      {!moment(attainmentDate).isSame(moment(completionDate), 'day') ? <Popup
        content={`Completion date is adjusted automatically to match the study right in the enrollment. Original completion date was ${moment(attainmentDate).format("DD.MM.YYYY")}`}
        trigger={<Icon name="pencil alternate" circular />} /> : null}
    </div>
    : null
        ...
        <Table.Body data-cy="confirm-entries-table">
          {rows.map(({ entry, ...rawEntry }) => {
            return <Table.Row key={entry.id} warning={entry.missingEnrolment} style={entry.type === 'EXTRA_ENTRY' ? styles.extraEntry : null}>
              ...
              <Table.Cell><CompletionDate attainmentDate={rawEntry.attainmentDate} completionDate={entry.completionDate} /></Table.Cell>
